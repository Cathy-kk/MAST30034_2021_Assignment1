...
```{r}
# we normalize TC with L-2 norm, plot all TC after normalization
par(mfrow=c(2,3))
...
Data are in range around 0 to 0.1.
...
```
```{r}
# visualize pixel value 5 and 1
par(mfrow=c(1,2))
par(mar=c(5.1, 4.1, 4.1, 4.1))
tmpSM_1.new<- matrix(rep(0, 441), 21,21)
tmpSM_1.new[c(2:6), c(2:6)] <- 1
tmpSM_1.new[2,2] <- 5
plot(tmpSM_1.new, border=NA,xlab="", ylab="")
plot(matrix(scale(as.vector(tmpSM_1.new)), 21,21))
(mean(as.vector(tmpSM_1.new)))
